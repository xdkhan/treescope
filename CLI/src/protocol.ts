// Wire types mirroring `Sources/TreescopeProtocol` (kept in sync with
// `Web/src/protocol.ts`). Messages use a `t` discriminator with flat fields,
// matching the server's custom Swift Codable.

export const DEFAULT_PORT = 50067;
export const PORT_SCAN_COUNT = 16;
...
export function capabilityNames(bits: number): string[] {
...
export function displayValue(v: AttributeValue): string {
  switch (v.t) {
    ...
    case "size": return `${v.v.width.toFixed(1)} x ${v.v.height.toFixed(1)}`;
    ...
    case "image": return `Image ${v.w}x${v.h}`;
    case "nested": return `{ ${v.v.length} fields }`;
  }
}
